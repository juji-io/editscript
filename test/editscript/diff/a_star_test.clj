  ...
  (testing "Testing A* with some nested vector data"
    ...
    (is (#{[[[0] :-] [[1] :+ [:f]] [[2] :r :d]]
         ...
         (get-edits (diff [[:d] [:e] :f]
                            [[:e] [:f] :d]))))
    (is (#{[[[0 0] :r :b] [[1] :-] [[1 1 0] :r :e] [[1 2 0] :r :f] [[1 3] :r :d]]
    ...
    (is (#{[[[0] :+ :s] [[1] :r :t]] [[[0] :r :s] [[1] :+ :t]]}
         (get-edits (diff [[:u]] [:s :t]))))
    (is (#{[[[0] :+ :b] [[1] :r :c] [[2] :r [:e]] [[4] :+ :g]]
           [[[0] :r :b] [[1] :+ :c] [[2] :r [:e]] [[4] :+ :g]]
           [[[0] :r :b] [[1] :r :c] [[2] :+ [:e]] [[4] :+ :g]]}
         (get-edits (diff [[:a [:b :c] :d] :e :f]
                          [:b :c [:e] :f :g]))))
    ...
    (is (#{[[[0 1] :-] [[1] :+ :t] [[2] :r :s]]
           [[[0 1] :-] [[1] :r :t] [[2] :+ :s]]}
         (get-edits (diff [[:s :t] [:u]] [[:s] :t :s]))))
    (is (= (get-edits (diff [:a [:s :t] :u]
                            [[:b] [:s :t :u]]))
           [[[0] :r [:b]] [[1 2] :+ :u] [[2] :-]]))
    (is (#{[[[0] :-] [[0 1] :r :u] [[1] :+ :t] [[2] :r :s]]
           [[[0] :-] [[0 1] :r :u] [[1] :r :t] [[2] :+ :s]]}
         (get-edits (diff [:a [:s :t] [:u]]
                          [[:s :u] :t :s]))))
    (is (#{[[[1 1] :+ :c] [[1 2] :r :d] [[2] :+ :e]]
           [[[1 1] :r :c] [[1 2] :+ :d] [[2] :+ :e]]}
         (get-edits (diff [:a [:b [:c [:d :e] :f]]]
                          [:a [:b :c :d] :e]))))))
